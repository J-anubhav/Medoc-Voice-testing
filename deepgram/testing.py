...
if DEEPGRAM_API_KEY:
    try:
        ...
        print(f"Error configuring Deepgram API: {e}")
        exit()
else:
    print("Error: DEEPGRAM_API_KEY not found in environment variables or .env file.")
    print("Please create a .env file in the same directory as this script and add the line: DEEPGRAM_API_KEY='your_api_key'")
    exit()
...
def format_diarized_transcript(response):
    """
    Format the diarized transcript into Doctor/Patient format.
    """
    try:
        # Extract results
        ...
        if not words:
            return "No words found in transcription"
        ...
        for word in words:
            speaker_id = getattr(word, 'speaker', 0)
            word_text = getattr(word, 'word', '')
            
            # Map speakers to roles - first speaker is Doctor, second is Patient
            ...
            if current_speaker != speaker_id:
                ...
                if current_speaker is not None and current_text.strip():
                    role = speaker_mapping[current_speaker]
                    formatted_transcript += f"{role}: {current_text.strip()}\n"
        ...
        print("Trying alternative formatting method...")
        
        # Fallback method
        try:
            # Try to get basic transcript
            transcript = results.channels[0].alternatives[0].transcript
            
            # Simple split by sentences and alternate speakers
            ...
            for i, sentence in enumerate(sentences):
                if sentence.strip():
                    ...
                    formatted_transcript += f"{speaker}: {sentence.strip()}\n"
            
            return formatted_transcript.strip()
            
        except Exception as e2:
            print(f"Fallback formatting also failed: {str(e2)}")
            # Return basic transcript without speaker labels as last resort
            try:
...
def transcribe_audio_only(audio_path):
    """
    Using Deepgram to extract transcription with diarization
    """
    ...
    try:
        ...
        # Configure options for medical transcription with diarization
        ...
        # Format the response
        formatted_transcript = format_diarized_transcript(response)
...
def run_testing_pipeline(audio_dir, groundtruth_dir):
    """
    Medical transcription model evaluation testing pipeline using Deepgram
    """
    results = []
    total_wer = 0
    total_precision = 0
    ...
    print("=" * 70)
    print("                 MEDICAL TRANSCRIPTION TESTING PIPELINE (DEEPGRAM)")
    print("=" * 70)
    ...
    if file_count > 0:
        ...
        if processed_count > 0:
            ...
            # Save main results
            output_filename = 'overall_evaluation_report_deepgram.csv'
